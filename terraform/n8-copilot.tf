...
locals {
  ...
  n8n_env  = terraform.workspace == "production" ? "production" : "testing"
}
...
resource "aws_db_instance" "n8n" {
  ...
  db_subnet_group_name   = local.is_prod ? aws_db_subnet_group.main[0].name : aws_db_subnet_group.test[0].name
