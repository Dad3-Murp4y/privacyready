...
export function ScoreCard({ score, change }: { score: number; change?: number }) {
  const safeScore = Math.max(0, Math.min(100, score));
  return <Card className="score-card"><div className="score-card__ring" style={{ '--score': `${safeScore * 3.6}deg` } as React.CSSProperties}><span>{safeScore}</span><small>/100</small></div><div><p className="eyebrow">Privacy readiness</p><h2>Your overall privacy posture</h2><p className="muted">An organisation-wide view based on your latest recorded scans and controls.</p>{typeof change === 'number' && <StatusBadge tone={change >= 0 ? 'success' : 'danger'}>{change >= 0 ? '+' : ''}{change} points since previous scan</StatusBadge>}</div></Card>;
}
...
export function FindingCard({ title, description, severity = 'warning' }: { title: string; description: string; severity?: 'danger' | 'warning' | 'info' }) {
  return <div className="finding-card"><StatusBadge tone={severity}>{severity === 'danger' ? 'Critical' : severity === 'warning' ? 'Review' : 'Info'}</StatusBadge><div><h3>{title}</h3><p>{description}</p></div></div>;
}
...
export function PageHeader({ eyebrow, title, description, actions }: { eyebrow?: string; title: string; description?: string; actions?: ReactNode }) {
  return <header className="page-header-v2"><div>{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h1>{title}</h1>{description && <p>{description}</p>}</div>{actions && <div className="page-header-v2__actions">{actions}</div>}</header>;
}
...
export function Skeleton({ className }: { className?: string }) { return <div className={cx('ui-skeleton', className)} aria-hidden="true"><LoaderCircle /></div>; }
