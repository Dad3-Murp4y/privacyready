...
export default function FindingList({ findings }: { findings: ScanFinding[] }) {
  ...
  return <div className="finding-list">{actionable.map((finding, index) => <FindingCard key={`${finding.finding_type || finding.title}-${index}`} title={finding.title || finding.finding_type?.replaceAll('_', ' ') || 'Finding'} description={finding.description || finding.detail || 'Review this finding in the scan evidence.'} severity={severity(finding)} badgeLabel={finding.severity ? finding.severity.charAt(0).toUpperCase() + finding.severity.slice(1).toLowerCase() : 'Review'} evidence={finding.evidence} remediation={finding.remediation} legalReference={finding.gdpr_article} />)}</div>;
}
