...
const pageProps = {
  about: {
    ...
    description: 'Learn how PrivacyReady helps UK SMEs assess website privacy signals and organise practical remediation and repeat checks.',
    canonicalPath: '/about',
  },
  contact: {
...
export function AboutPage() {
  return <PublicSiteLayout {...pageProps.about} mainClassName="public-page"><PageIntro eyebrow="About PrivacyReady" title="Practical privacy assessment and remediation for UK SMEs" lead="PrivacyReady helps small and mid-sized organisations identify observable website privacy risks, understand what needs attention and organise practical remediation." /><section><h2>Our purpose</h2><p>Privacy obligations apply to organisations of every size, but the work can be difficult to organise without specialist resources. PrivacyReady brings website assessments, prioritised findings, remediation guidance and operational records into one focused workspace.</p><p>The software supports privacy readiness and practical decision-making. It does not provide legal advice, guarantee compliance, certify fixes or replace advice from a qualified professional.</p></section><section><h2>How the platform helps</h2><div className="public-card-grid"><article><h3>Assess observable signals</h3><p>Scan public web pages for transport security, privacy information, cookies, scripts and data-collection indicators that may require review.</p></article><article><h3>Understand and address</h3><p>Use supported finding context and remediation guidance to prioritise practical action without inventing evidence or hiding uncertainty.</p></article><article><h3>Re-scan after changes</h3><p>Run another assessment and compare results to check whether previously detected website signals have changed.</p></article></div></section><section><h2>Company information</h2><p>PrivacyReady Ltd (Reg No. 14592031) is registered in England &amp; Wales.</p><p>Questions? <Link to="/contact">Contact PrivacyReady</Link>.</p></section></PublicSiteLayout>;
}
...
export function CookiesPage() {
  return <PublicSiteLayout {...pageProps.cookies} mainClassName="public-page public-page--legal"><PageIntro eyebrow="Legal" title="Cookies and browser storage" lead="This page describes the cookies and browser storage used by the current PrivacyReady application." /><section><h2>1. Authentication cookie</h2><p>When you sign in, PrivacyReady sets the <code>__Host-token</code> cookie to authenticate your session. It is HttpOnly, uses SameSite=Lax, is marked Secure in production and expires after one hour. It is necessary for signed-in portal functionality.</p></section><section><h2>2. Browser storage</h2><p>The public scan journey uses same-tab session storage for <code>freeScanId</code>, <code>freeScanClaimToken</code>, <code>freeScanUrl</code> and <code>freeScanScore</code> while you move between the scanner, registration and sign-in. If a signed-in scan claim fails, <code>scanClaimError</code> stores a same-tab guidance message. Legacy persistent copies from earlier versions are removed when registration opens. These values are browser storage, not cookies or analytics identifiers.</p></section><section><h2>3. Analytics and advertising</h2><p>PrivacyReady does not currently load optional analytics, advertising or behavioural-tracking technology in the portal. There is therefore no analytics consent preference to manage in the current application.</p></section><section><h2>4. Clearing stored data</h2><p>You can clear cookies and site data using your browser controls. Signing out clears the authentication cookie and also expires the legacy <code>auth_payload</code> cookie if an older version of the application set it.</p></section></PublicSiteLayout>;
}
