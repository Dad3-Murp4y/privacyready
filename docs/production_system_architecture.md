...
The DataWai Production Environment (`terraform.workspace == "production"`) is a highly available, deeply isolated, and scalable architecture designed for enterprise-grade PDPA compliance. Unlike the consolidated testing environment, the production environment heavily utilizes network segmentation to separate management tools from user-facing services.
